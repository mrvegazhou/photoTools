...
function CanvasRuler(params, dom) {
  ...
  this.setCanvasHeigth = function(height) {
  ...
  this.setCanvasWidth(params.width);
  this.setCanvasHeigth(params.height);
}
...
CanvasRuler.prototype = {
  ...
  drawHorizontalAxis: function(ctx) {
    ...
    ctx.stroke();
    for (let i = gridWidth + 0.5; i < this.getCanvasWidth(); i = i + gridWidth) {
      ctx.beginPath();
      ctx.moveTo(i, 0);
  ...
  drawVerticalAxis: function(ctx) {
    ...
    ctx.stroke();
    for (let i = gridWidth + 0.5; i < this.getCanvasHeight(); i = i + gridWidth) {
      ctx.beginPath();
      ctx.moveTo(0, i);
