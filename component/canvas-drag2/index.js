...
const dragGraph = function ({x = 30, y = 30, w, h, type, text, mode='horizontal', fontSize = 20, color = 'red', url = null, rotate = 0, sourceId = null, selected = true}, canvas, factor) {
    if (type === 'text') {
...
dragGraph.prototype = {
    ...
    drawVerticalText() {
      ...
      let maxLw = 0, len = strArr.length;
      for(var i = 0; i<len; i++) {
        // maxW = Math.max(maxW, this.ctx.measureText(strArr[i]).width)
        maxLw = Math.max(maxLw, this.ctx.measureText(strArr[i][0]).width) + 2
      }
      // let cx = this.x + maxLw*len/2
      // let cy = this.y + maxW/2
      let leftStep = 0
      for(var i = 0; i<len; i++) {
        let left = this.x + leftStep
        let top = this.y
        util.drawVerticalText4Canvas(this.ctx, strArr[i], left, top)
        leftStep += maxLw
      }
    },
    ...
    paint() {
        ...
        if (this.selected) {
            ...
            if (this.type === 'text') {
                ...
                this.ctx.drawImage(DELETE_ICON, this.x - 10, this.y - 10, 20, 20);
                console.log(this.x + textWidth - 10, this.y + textHeight - 10)
                this.ctx.drawImage(DRAG_ICON, this.x + textWidth - 10, this.y + textHeight - 10, 20, 20);
            } else {
                this.ctx.strokeRect(this.x, this.y, this.w, this.h);
                this.ctx.drawImage(DELETE_ICON, this.x - 10, this.y - 10, 20, 20);
                this.ctx.drawImage(DRAG_ICON, this.x + this.w - 10, this.y + this.h - 15, 20, 20);
            }
        }
        this.ctx.restore();
    },
